...
export default function TextractPage() {
  ...
  const [results, setResults] = useState<TextractResults | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [humanReviewRequired, setHumanReviewRequired] = useState(false);
  const [humanReviewUrl, setHumanReviewUrl] = useState<string | null>(null);
    ...
    const checkApi = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setApiStatus(`API is available (${data.time})`);
        } else {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // First try the test endpoint to make sure API routing is working
      try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          errorMessage = errorData.error || errorData.message || 'Failed to process file';
        } catch (parseErr) {
          // If not valid JSON, use the text directly
          errorMessage = `Response not JSON (${response.status}): ${errorText.substring(0, 100)}...`;
        }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          // Try to parse as JSON
          const errorData = JSON.parse(errorText);
          errorMessage = errorData.error || errorData.message || 'Failed to reprocess file';
        } catch (parseErr) {
          // If not valid JSON, use the text directly
          errorMessage = `Response not JSON (${response.status}): ${errorText.substring(0, 100)}...`;
        }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          // Try to parse as JSON
          const errorData = JSON.parse(errorText);
          errorMessage = errorData.error || errorData.message || 'Failed to start training';
        } catch (parseErr) {
          // If not valid JSON, use the text directly
          errorMessage = `Response not JSON (${response.status}): ${errorText.substring(0, 100)}...`;
        }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <main className="container mx-auto py-10 px-4 max-w-6xl">
        ...
        {apiStatus && (
          <div className={`mb-4 p-2 text-sm rounded-md ${apiStatus.includes('not available') ? 'bg-red-50 text-red-700' : 'bg-green-50 text-green-700'}`}>
